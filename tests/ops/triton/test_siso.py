...
def test_siso_step(nheads_qk=4, has_Z=True, has_D=True):
    ...
    for step in range(seqlen):
        ...
        out_triton, output_states_triton = siso_step(
    ...
    outputs_ref, final_states_ref = siso_step_ref(
...
def test_siso_combined_batched(nheads_qk=4, has_Z=True, has_D=True, headdim_qk=128):
    ...
    # Reference: use siso_fwd_ref to compute full sequence output.
    Out_ref, Final_States_ref = siso_fwd_ref(
    ...
    Out_first, Angle_State_1, SSM_State_1, K_State_1, V_State_1 = siso_combined(
    ...
    Out_second, Final_Angle_State, Final_SSM_State, Final_K_State, Final_V_State = siso_combined(
        inputs['Q'][:, half:], inputs['K'][:, half:], inputs['V'][:, half:],
        inputs['ADT'][:, :, half:], inputs['DT'][:, :, half:], inputs['Trap'][:, :, half:],
        inputs['Q_bias'], inputs['K_bias'], inputs['Angles'][:, half:],
        inputs['D'], inputs['Z'][:, half:] if has_Z else None,
...
# The error for angle is ~20% because cumsum accumulates error over sequence length. This
# error becomes ~3% when cumsum (angle-dt) kernel is removed
def test_siso_combined_varlen(nheads_qk=4, has_Z=True, has_D=True, headdim_qk=128):
    ...
    # Reference: use siso_fwd_ref with cu_seqlens
    Out_ref, Final_States_ref = siso_fwd_ref(
        inputs_ref['Q'], inputs_ref['K'], inputs_ref['V'],
        inputs_ref['ADT'], inputs_ref['DT'], inputs_ref['Trap'],
        inputs_ref['Q_bias'], inputs_ref['K_bias'], inputs_ref['Angles'],
    ...
    Out_kernel, Final_Angle_State, Final_SSM_State, Final_K_State, Final_V_State = siso_combined(
...
def test_siso_step_ref_vs_fwd_ref(nheads_qk=4, has_Z=True, has_D=True):
    """Test that siso_step_ref and siso_fwd_ref produce identical outputs."""
    device = 'cuda'
    ...
    # --- Step ref ---
    out_step, final_states_step = siso_step_ref(
    ...
    # --- Fwd ref ---
    out_fwd, final_states_fwd = siso_fwd_ref(
        inputs['Q'], inputs['K'], inputs['V'],
        inputs['ADT'], inputs['DT'], inputs['Trap'],
        inputs['Q_bias'], inputs['K_bias'], inputs['Angles'],
        inputs['D'], inputs['Z'],
