...
#
# AUM-Ø U phase ("unfold"): resonant AFFINE evidence recurrence (§6). Forked from Mamba-3 SISO.
# TODO(AUM): (1) add write-gate rho_t = sigmoid(r_t) on the state write; (2) L2-normalize k_hat/v_hat;
# (3) retire the MIMO/tilelang/cute paths (kept guarded below; they resolve to None in this fork).
# The phase recurrence phi_t = (phi_{t-1} + pi*tanh(theta)*tau) mod 2pi is already provided by
# aum_ssm/ops/triton/unfold/angle_dt.py — the closest existing analog of the resonance clock.
...
from aum_ssm.ops.triton.layernorm_gated import RMSNorm as RMSNormGated

try:
    from aum_ssm.ops.tilelang.mamba3.mamba3_mimo import mamba3_mimo as mamba3_mimo_combined
except ImportError:
    mamba3_mimo_combined = None

from aum_ssm.ops.triton.unfold.siso_combined import siso_combined

from aum_ssm.ops.triton.unfold.rotary_step import apply_rotary_qk_inference_fwd

try:
    from aum_ssm.ops.cute.mamba3.mamba3_step_fn import mamba3_step_fn
except ImportError:    
    mamba3_step_fn = None
