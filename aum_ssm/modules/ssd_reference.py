...
def _rotate_single_phase(x, phi):
    """Rotate every adjacent pair of the last dim by the scalar phase phi (§6, R(phi)).

    x: (..., D) with D even; phi: (...,) broadcast over the D/2 pairs.
    """
...
def aum_unfold_step_ref(q, k, v, tau_bar, lam_bar, r, theta, z=None, D=None,
    ...
    """Serial (recurrent) reference for the §6 U phase.

    q, k: (B, L, H, Dqk)   v, z: (B, L, H, Dv)
    tau_bar, lam_bar, r, theta: (B, L, H)   D: (H,) or (H, Dv)
    ...
    for t in range(L):
        ...
        k_hat = _l2norm(k[:, t])
        v_hat = _l2norm(v[:, t])
        q_rot = _rotate_single_phase(q[:, t], phi)
        k_rot = _rotate_single_phase(k_hat, phi)
...
def aum_unfold_chunk_ref(q, k, v, tau_bar, lam_bar, r, theta, z=None, D=None,
                         dt_bias=0.0, eps=1e-4, block_len=None, norm_weight=None):
    """Chunk-parallel reference for the §6 U phase (segsum factorization).
    ...
    q_rot = _rotate_single_phase(q, phi)
